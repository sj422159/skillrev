...
<form action="{{url('supervisor/studentdetails/bysection')}}" method="post">
    @csrf
    <div class="form-row">
        ...
        <div class="col-12 col-sm-3 mt-2 mt-sm-0" style="display: flex;justify-content: center;flex-direction: column;align-items: center;" >
            ...
            @if(count($details)>0)
                <a href="{{url('supervisor/studentdetails/export')}}/{{$class}}/{{$section}}"><button type="button" class="btn btn-primary">Export</button>
                </a>
            @else
            ...
            <table class="table table-borderless table-data3">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            @if($list->status==1)
                                  <a href="{{url('supervisor/student/status/0')}}/{{$list->id}}"><button type="button" class="btn btn-sm btn-primary">Active</button>
                                  </a> 
                               @elseif($list->status==0)
                                 <a href="{{url('supervisor/student/status/1')}}/{{$list->id}}"><button type="button" class="btn btn-sm btn-warning">Deactive</button>
                                  </a>
                               @endif
                        </td>
                        <td> 
                            <a href="{{url('supervisor/student/delete')}}/{{$list->id}}"><button type="button" class="btn btn-sm btn-danger">Delete</button>
                            </a>
                        </td>
                    </tr>
                    @endforeach
