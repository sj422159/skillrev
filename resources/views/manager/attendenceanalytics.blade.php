...
<form action="{{url('manager/analytics/attendance/fetch')}}" method="post">
    @csrf
    <div class="form-row">
          <div class="col-12 col-sm-4 mt-2 mt-sm-0" style="display: flex;justify-content: center;flex-direction: column;align-items: center;">
            <label>CLASS</label>
            <select  class="form-control disabled" required  name="class" id="class" data-val="{{$section}}" required disabled>
...
function  reload(that) {
    ...
    $.ajax({
              url:'{{url("manager/analytic/attendance/fetch/datewise")}}',
              type:'GET',
