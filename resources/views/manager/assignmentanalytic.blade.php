...
<form action="{{url('manager/analytics/assignment/fetch')}}" method="post">
    @csrf
    <div class="form-row">
          <div class="col-12 col-sm-3 mt-2 mt-sm-0" style="display: flex;justify-content: center;flex-direction: column;align-items: center;">
            <label>CLASS</label>
            <select  class="form-control disabled" required  name="class" id="clas" data-val="{{$section}}" required onchange="sec(this)" readonly>
...
function  reload(that) {
    ...
    if(type==1){
        ...
        $.ajax({
              url:'{{url("manager/analytic/assignment/notcompleted")}}',
              type:'GET',
        ...
        $.ajax({
              url:'{{url("manager/analytic/assignment/completed")}}',
              type:'GET',
              data:{assignment:assignment},
              dataType: "json",
              success:function(sdata){
...
<script type="text/javascript">
  ...
  function sec(that){
          ...
          var sectionid=$('#clas').attr('data-val');
           ...
           $('#section').html('');
            $.ajax({
              url:'{{url("manager/classby/section/{id}")}}',
              type:'GET',
              data:{id:classid},
              dataType: "json",
