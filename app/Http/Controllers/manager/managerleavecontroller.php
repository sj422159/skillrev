...
class managerleavecontroller extends Controller
{
    public function leave(Request $request){
        ...
        $result['leave']=DB::table('leaves')->where('profileid',$mid)->where('portalid',2)->latest('id')->get();
        return view('manager.leave',$result);
    }
    ...
    public function addleave(Request $request,$id=""){   
        ...
        return view("manager.addleave",$result);
    }
    ...
    public function saveleave(Request $request){
        ...
        $model->portalid=2;
        $model->profileid=session()->get('MANAGER_ID');
