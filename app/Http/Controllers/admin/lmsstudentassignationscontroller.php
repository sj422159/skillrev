...
class lmsstudentassignationscontroller extends Controller
{
  public function reports()
  {
    ...
    // Fetch common data and filter based on the user type
    $d = DB::table('admins')->where("id", $aid)->get();

    if ($aid) {
      // Fetch trainings and categories for the logged-in admin
      ...
      // Fetch trainings and categories for the logged-in controller
  ...
  public function classby()
  {
    ...
    // Fetch sections based on class ID and user type (admin or controller)
    if ($aid) {
      // Fetch sections for the class created by the logged-in admin
      ...
      // No valid session; return an empty response
  ...
  public function fetchstu(Request $request) {
    ...
    // Fetch admin or controller details
    $userDetails = DB::table('admins')
        ->where("id", $aid ?: $controller_id)
        ->get();

    // Fetch trainings created by the logged-in admin or controller
    $result['train'] = DB::table('trainings')
        ->where($aid ? 'aid' : 'controller_id', $aid ?: $controller_id)
        ->where('status', 1)
        ->get();

    // Initialize result variables from the request
    ...
    // Fetch student booking data based on the creator (admin or controller)
    $result['data'] = DB::table('studentbookings')
        ...
        ->where('studentbookings.' . ($aid ? 'aid' : 'controller_id'), $aid ?: $controller_id)
        ->select(
            'studentbookings.*',
            'students.sname',
            'students.slname',
